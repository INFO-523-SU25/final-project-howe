...
-   Columns of interest include: 'Entity' (state), 'type_of_crime', and 'count'. To condense the dataset, I filtered out all rows with a crime count of 0, and reduced the scope to the years 2020-2022.
...
As for the lack of correlation between the number of cartel's per state and the crime count, I find it necessary to dive deeper before making a clear assessment. Some data to take into further consideration would be the dominion of each cartel, as well as the size of each state. For example, Guerero has 12 associated cartels with 3.5x less crime than Nuevo Leon, a state with only one cartel organization present. It could be that the cartel in Nuevo Leon is a much larger organization that does not allow for other cartels to inhabit the area.
